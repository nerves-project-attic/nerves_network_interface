...
permissions on the `nerves_network_interface` binary, so you'll be asked your password towards
...
`mix.exs` deps list:
     ...
     {:nerves_network_interface, github: "fhunleth/nerves_network_interface.ex", branch: "master"}
...
The next step is to create a `Nerves.NetworkInterface` process. By default, `NetBasic` creates
a GenEvent process for notifying interface change events, but you can supply one
as an argument to `Nerves.NetworkInterface.start_link/1`.

    iex> {:ok, pid} = Nerves.NetworkInterface.start_link
    {:ok, #PID<0.82.0>}

To see which interfaces are available, call `Nerves.NetworkInterface.interfaces\0`:

    iex> Nerves.NetworkInterface.interfaces pid
	['lo', 'eth0', 'wlan0']

To get link-level status information and statistics on an interface, call
`Nerves.NetworkInterface.status/2`:

    iex> Nerves.NetworkInterface.status(pid, "eth0")
...
register a `GenEvent` with `Nerves.NetworkInterface`. If you don't supply one in the call
to `start_link`, one is automatically created and available via `Nerves.NetworkInterface.event_manager/1`. The
following example shows how to view events at the prompt:
    ...
    ...>  end
    ...> end
    iex> Nerves.NetworkInterface.event_manager(pid) |> GenEvent.add_handler(Forwarder, self())
    :ok
    iex> flush
    :ok
...
To get the IP configuration for an interface, call `Nerves.NetworkInterface.settings/2`:

    iex> Nerves.NetworkInterface.settings(pid, "eth0")
...
`Nerves.NetworkInterface.setup/3` using keyword parameters or a map with what you'd like
to set. The following example uses keyward parameters:

    iex> Nerves.NetworkInterface.setup(pid, "eth0", ipv4_address: "192.168.25.200",
...
`Nerves.NetworkInterface.ifdown/1`. As you would expect, these require privilege to run:
    ...
    iex> Nerves.NetworkInterface.ifdown(pid, "eth0")
    :ok
